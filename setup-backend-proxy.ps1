# PowerShell script to set up backend proxy in IIS
# This configures URL Rewrite to proxy /api/* to backend on port 4000
# Run this script as Administrator

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "WeFix Backend Proxy Setup" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
$existingApiRules = @($rulesNode.rule | Where-Object { $_.name -eq "Proxy GraphQL API" })
foreach ($rule in $existingApiRules) {
    $rulesNode.RemoveChild($rule) | Out-Null
}

# Add API proxy rule (at the beginning, before SPA rule)
...
if ($rulesNode.ChildNodes.Count -gt 0) {
    $rulesNode.InsertBefore($apiRule, $rulesNode.FirstChild) | Out-Null
} else {
    $rulesNode.AppendChild($apiRule) | Out-Null
}

# Ensure SPA routing rule exists
...
Write-Host "Backend Proxy Setup Complete!" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
Write-Host ""
...
Write-Host "  2. Update backend CORS to allow http://localhost" -ForegroundColor White
Write-Host "  3. Test the API at: http://localhost/api/graphql" -ForegroundColor White
Write-Host ""
